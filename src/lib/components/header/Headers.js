...
export default class Header extends PureComponent {
	...
	// 			else return 'MMMM';
	// 		case 'week':
	// 			if (position == 'top') return 'ww MMMM YYYY TEST';
	// 			else return 'ww';
	// 		case 'dayweek':
	// 			return 'LLLL';
	// 		case 'daymonth':
	...
	// handleMomentFormat(momentInstance, mode, position) {
	// 	switch (mode) {
	// 		case 'year':
	// 			return momentInstance.format('YYYY');
	// 		case 'month':
	// 			if (position == 'top') return momentInstance.format('MMMM YYYY');
	// 			else return momentInstance.format('MMMM');
	// 		case 'week':
	// 			if (position == 'top') {
	// 				const startOfTheWeek = momentInstance.startOf('week').format('LL');
	// 				const endOfTheWeek = momentInstance.endOf('week').format('LL');
	// 				return startOfTheWeek + ' - ' + endOfTheWeek;
	// 			} else return momentInstance.format('ww');
	// 		case 'dayweek':
	...
	formatHeader(date, mode, position) {
		debugger;
		switch (mode) {
			case 'year':
				return date.format('YYYY');
			case 'month':
				if (position == 'top') return date.format('MMMM YYYY');
				else return date.format('MMMM');
			case 'week':
				if (position == 'top')
					return `${date.startOf('isoWeek').format('L')} - ${date
						.endOf('isoWeek')
						.format('L')} ${date.format('MMMM YYYY')}`;
				else return date.format('ww');
			case 'dayweek':
	...
	renderHeaderRows = (top, middle, bottom) => {
		...
		for (let i = start - BUFFER_DAYS; i < end + BUFFER_DAYS; i++) {
			...
			if (currentTop != this.formatHeader(currentDate, top, 'top')) {
				currentTop = this.formatHeader(currentDate, top, 'top');
			...
			if (currentMiddle != this.formatHeader(currentDate, middle)) {
				currentMiddle = this.formatHeader(currentDate, middle);
			...
			if (currentBottom != this.formatHeader(currentDate, bottom)) {
				currentBottom = this.formatHeader(currentDate, bottom);
		...
		// 	//The unit of iteration is day
		// 	currentDate = moment().add(i, 'days');
		// 	const formatedCurrentTopDate = this.handleMomentFormat(
		// 		currentDate,
		// 		top,
		// 		'top'
		// 	);
		// 	// if (currentTop != currentDate.format(this.getFormat(top, 'top'))) {
		// 	if (currentTop != formatedCurrentTopDate) {
		// 		currentTop = formatedCurrentTopDate;
		// 		box = this.getBox(currentDate, top, lastLeft.top);
		// 		lastLeft.top = box.left + box.width;
		// 		result.top.push(
		// 			<HeaderItem
		...
		// 	const formatedCurrentMiddleDate = this.handleMomentFormat(
		// 		currentDate,
		// 		middle
		// 	);

		// 	if (currentMiddle != formatedCurrentMiddleDate) {
		// 		currentMiddle = formatedCurrentMiddleDate;
		// 		box = this.getBox(currentDate, middle, lastLeft.middle);
		// 		lastLeft.middle = box.left + box.width;
		// 		result.middle.push(
		// 			<HeaderItem
		...
		// 	const formatedCurrentBottomDate = this.handleMomentFormat(
		// 		currentDate,
		// 		bottom
		// 	);

		// 	if (currentBottom != formatedCurrentBottomDate) {
		// 		currentBottom = formatedCurrentBottomDate;
		// 		box = this.getBox(currentDate, bottom, lastLeft.bottom);
		// 		lastLeft.bottom = box.left + box.width;
		// 		if (bottom == 'shorttime' || bottom == 'fulltime') {
		// 			result.bottom.push(this.renderTime(box.left, box.width, bottom, i));
